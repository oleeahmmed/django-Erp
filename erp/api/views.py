...
from .serializers import (
    ...
    ProductionReceiptSerializer, ProductionReceiptComponentSerializer,
...
class ProductionReceiptComponentViewSet(viewsets.ModelViewSet):
    queryset = ProductionReceiptComponent.objects.select_related('product').all()
    serializer_class = ProductionReceiptComponentSerializer
    permission_classes = [DjangoModelPermissions]
...
from erp.models import QuickSale, QuickSaleItem, UserPOSProfile
from .serializers import QuickSaleSerializer, QuickSaleItemSerializer, QuickSaleCreateSerializer, UserPOSProfileSerializer
from rest_framework.decorators import action
from rest_framework.response import Response
from rest_framework import status


class UserPOSProfileViewSet(viewsets.ModelViewSet):
    queryset = UserPOSProfile.objects.select_related('user', 'default_customer', 'default_warehouse').all()
    serializer_class = UserPOSProfileSerializer
    permission_classes = [DjangoModelPermissions]
    filter_backends = [filters.SearchFilter]
    ...
    def my_profile(self, request):
        ...
        try:
            profile = UserPOSProfile.objects.get(user=request.user)
            serializer = self.get_serializer(profile)
            return Response(serializer.data)
        except UserPOSProfile.DoesNotExist:
            return Response({'detail': 'POS Profile not found'}, status=status.HTTP_404_NOT_FOUND)
