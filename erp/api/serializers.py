...
class ProductionReceiptComponentSerializer(serializers.ModelSerializer):
    product_name = serializers.CharField(source='product.name', read_only=True)
...
class ProductionReceiptSerializer(serializers.ModelSerializer):
    components = ProductionReceiptComponentSerializer(many=True, read_only=True)
...
class UserPOSProfileSerializer(serializers.ModelSerializer):
...
class QuickSaleCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Get warehouse and customer from user's POS profile
        from erp.models import UserPOSProfile, Warehouse
        try:
            profile = UserPOSProfile.objects.get(user=user)
            if profile.default_warehouse:
                validated_data['warehouse'] = profile.default_warehouse
            ...
            if first_wh:
                validated_data['warehouse'] = first_wh
