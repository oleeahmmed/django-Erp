...
# Status: completed → Finished Product IN + BOM Components OUT
...
def production_receipt_post_save(sender, instance, **kwargs):
        ...
        if not is_already_processed(reference):
            ...
            if qty > 0:
                create_stock_transaction(
                    ...
                    quantity=qty,
                    reference=reference,
                    notes=f"Production of {po.product.name}"
                )
            
            if po.bom:
                for comp in po.bom.components.all():
                    comp_qty = comp.quantity * qty * (Decimal('1') + comp.scrap_percentage / Decimal('100'))
                    create_stock_transaction(
                        ...
                        quantity=comp_qty,
                        reference=reference,
                        notes=f"Consumed for {po.product.name}"
                    )
            
            po.quantity_produced += qty
    ...
    if instance.status == 'completed' and old_status != 'completed':
        ...
        po = instance.production_order
        po.quantity_produced -= instance.quantity_received
...
def quick_sale_pre_save(sender, instance, **kwargs):
    instance._old_status = None
    if instance.pk:
        try:
            instance._old_status = sender.objects.get(pk=instance.pk).status
...
def quick_sale_post_save(sender, instance, **kwargs):
    from django.db import transaction as db_transaction
    
    old_status = getattr(instance, '_old_status', None)
    reference = f"QS:{instance.sale_number}"
        ...
        if not is_already_processed(reference):
            for item in instance.items.all():
                create_stock_transaction(
                    ...
                    quantity=item.quantity,
                    reference=reference,
                    notes=f"Quick Sale to {instance.customer_name or 'Walk-in'}"
                )
    
    if instance.status == 'completed' and old_status != 'completed':
        db_transaction.on_commit(process_sale)
